...
namespace agoNetwork {
    ...
    noexcept(false) {
        if (not _socket.name.empty() && not _socket.address.empty()) {
            if (validateURI_(_socket.address)) {
                ...
                throw (std::runtime_error("Could not validate " + _socket.address + "\nvalid uri: ipv4:port"));
            }
        }
    }
            ...
            auto _ = std::async(std::launch::async, [&] {
                for (const auto &[socketName, socket] : _tcpSocket) {
                    try {
                        ...
                        std::cout << "Error in binding to tcp socket " << socketName << ", what? " << error.what()
                                  << std::endl;
                    }
                }
            });
            auto __ = std::async(std::launch::async, [&] {
                for (const auto &[socketName, socket] : _ipcSocket) {
                    try {
                        ...
                        std::cout << "Error in binding to ipc socket " << socketName << ", what? " << error.what()
                                  << std::endl;
                    }
                }
            });
            auto ___ = std::async(std::launch::async, [&] {
                for (const auto &[socketName, socket] : _inprocSocket) {
                    try {
                        ...
                        std::cout << "Error in binding to inproc socket " << socketName << ", what? " << error.what()
                                  << std::endl;
                    }
                }
            });
                    ...
                    try {
                        ...
                        for (const auto &item : polls) {
                            ...
                            if (item.revents == ZMQ_POLLIN) {
                                auto _socket = _tcpSocket[socketPairPoll[socketIndex]];
                                auto req = _socket->receive();
                                if (_tcpCallbacks.contains(socketPairPoll[socketIndex])) {
                                    auto[rangeBegin, rangeEnd] = _tcpCallbacks.equal_range(socketPairPoll[socketIndex]);
                                    for (auto callback = rangeBegin; callback != rangeEnd; ++callback) {
                ...
                while (listeningOnIpc_()) {
                    try {
                        ...
                        for (const auto &item : polls) {
                            ++socketIndex;
                            if (item.revents == ZMQ_POLLIN) {
                                auto _socket = _ipcSocket[socketPairPoll[socketIndex]];
                                auto req = _socket->receive();
                                if (_ipcCallbacks.contains(socketPairPoll[socketIndex])) {
                                    auto[rangeBegin, rangeEnd] = _ipcCallbacks.equal_range(socketPairPoll[socketIndex]);
                                    for (auto callback = rangeBegin; callback != rangeEnd; ++callback) {
                                        callback->second(_socket, req);
                                    }
                                }
                ...
                while (listeningOnInproc_()) {
                    try {
                        ...
                        for (const auto &item : polls) {
                            ++socketIndex;
                            if (item.revents == ZMQ_POLLIN) {
                                auto _socket = _inprocSocket[socketPairPoll[socketIndex]];
                                auto req = _socket->receive();
                                if (_inprocCallbacks.contains(socketPairPoll[socketIndex])) {
                                    auto[rangeBegin, rangeEnd] = _inprocCallbacks.equal_range(socketPairPoll[socketIndex]);
                                    for (auto callback = rangeBegin; callback != rangeEnd; ++callback) {
                                        callback->second(_socket, req);
                                    }
                                }
    ...
    void router::status_(router::routerStatus &&status) noexcept {
        switch (_status) {
    ...
    bool router::bound_() const noexcept {
        return _status == routerStatus::bound;
    }

    bool router::listening_() const noexcept {
        return _status == routerStatus::listening;
    }

    bool router::listeningOnTcp_() const noexcept {
        return
    ...
    bool router::listeningOnIpc_() const noexcept {
        return
    ...
    bool router::listeningOnInproc_() const noexcept {
        return
    ...
    void router::listen() noexcept {
        listen_();
    }

    void router::registerCallback_(const std::string &name, const router::tcp_callback &callback) noexcept {
    ...
    void router::registerCallback_(const std::string &name, const router::ipc_callback &callback) noexcept {
    ...
    void router::registerCallback_(const std::string &name, const router::inproc_callback &callback) noexcept {
