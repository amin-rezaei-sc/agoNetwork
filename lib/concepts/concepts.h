...
// routerCallback -> std::invocable void (std::shared_ptr<agoNetwork::socket>, const std::vector<std::string> &)
template<typename... Callback>
concept routerCallback = (
        (std::is_invocable_r_v<void, Callback(const std::shared_ptr<agoNetwork::socket>&, const std::vector<std::string> &), std::shared_ptr<agoNetwork::socket>, const std::vector<std::string> &>
        ) && ...);
